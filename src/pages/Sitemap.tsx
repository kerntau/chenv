...
import { BlurText, CountUp, FadeContent } from '../components/reactbits';
      ...
      <Container size="wide">
        ...
        <article className="paper-sheet-realistic p-6 sm:p-10 md:p-12 font-sans space-y-10 my-4 sm:my-8 text-slate-800 dark:text-slate-200">
          ...
          <header className="text-center pb-5 sm:pb-6 border-b border-slate-200/70 dark:border-slate-800/70 space-y-2">
            <BlurText
              text="全站索隐与架构导览"
              className="font-serif text-xl sm:text-2xl font-bold text-slate-950 dark:text-slate-50 tracking-tight justify-center"
              delay={40}
              animateBy="words"
              direction="top"
            />
            ...
            <div className="pt-1 flex items-center justify-center space-x-3 text-[11px] font-mono text-slate-400 dark:text-slate-500">
              <span>手记 <CountUp to={allDiaries.length} duration={1} /> 篇</span>
              <span>&bull;</span>
              <span>说说 <CountUp to={allRecords.length} duration={1} /> 则</span>
              <span>&bull;</span>
              <span>友人 <CountUp to={allFriends.length} duration={1} /> 位</span>
            </div>
          </header>

          {/* 第一编：全站核心经纬目次 (Core Sections) */}
          <FadeContent delay={0.05} direction="up" distance={15} duration={0.4}>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-x-8 gap-y-1.5">
                  {allDiaries.map((diary) => (
                    <Link
                      ...
                      className="group flex items-baseline justify-between py-1.5 px-2 -mx-2 rounded-md hover:bg-slate-100/60 dark:hover:bg-slate-800/40 transition-colors"
                    >
          ...
          <FadeContent delay={0.15} direction="up" distance={15} duration={0.4}>
