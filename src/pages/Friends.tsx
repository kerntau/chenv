  ...
  return (
    <PageShell>
      <Container size="wide">
        ...
        {/* 朋友卡片 Grid (一行 4 个) */}
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-3 mb-6 sm:mb-10">
        ...
        {/* 申请友链指南 */}
        <div className="p-4 sm:p-5 rounded-lg border border-slate-200/70 dark:border-slate-800/70 space-y-3 bg-slate-50/50 dark:bg-[#18181A]/50 shadow-2xs">
          ...
          <div className="relative p-3.5 rounded-md bg-white/75 dark:bg-slate-900/75 border border-slate-200/60 dark:border-slate-800/60 text-xs font-mono text-slate-600 dark:text-slate-400 space-y-1 group">
            <button
              type="button"
...
const FriendCard: React.FC<{ friend: FriendItem }> = ({ friend }) => {
  ...
  return (
    <a
      ...
      target="_blank"
      rel="noreferrer"
      className="p-3 sm:p-3.5 rounded paper-card flex items-center space-x-3 group h-[74px] transition-all"
    >
      {/* 左侧头像 */}
      <div className="w-10 h-10 rounded-sm bg-slate-100 dark:bg-slate-800 text-slate-900 dark:text-slate-100 flex items-center justify-center font-sans text-xs font-bold shrink-0 overflow-hidden border border-slate-200/60 dark:border-slate-700/60">
      ...
      <div className="flex-1 min-w-0 flex flex-col justify-center">
        ...
        {/* 简介文本（单行截断，保证所有名片卡片高度像素级等高对齐） */}
        <p
          className="text-[10.5px] text-slate-500 dark:text-slate-400 mt-1 truncate font-sans"
          title={friend.desc}
        >
          {friend.desc || '独立个人博客'}
        </p>
